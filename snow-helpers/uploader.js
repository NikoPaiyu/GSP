...
fs.readdir("./dist/gsp",function(err,files){
  ...
  for (var i=0; i < files.length; i++){
    ...
    if (file.match(/^index.html$/g)){
      indexFile = file;
      console.log ("Inded-->"+config.files.html.index)
...
function upload_file(file,table,sys_id){
  ...
  var body = {};
  body[fields[table]] = fs.readFileSync('./dist/gsp/' + file).toString();
  ...
  var req = http.request(options, function(res) {
    ...
    if (statusCode !== 200) {
      console.log("Upload failed with status code " + statusCode + " for file:",file+"merror"+res.statusMessage);
      return;
    }
